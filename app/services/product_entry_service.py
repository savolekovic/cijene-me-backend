...
class ProductEntryService:
    ...
    async def get_all_entries(self) -> List[ProductEntry]:
        try:
    ...
    async def get_entry(self, entry_id: int) -> ProductEntry:
        try:
            logger.info(f"Fetching price entry with id: {entry_id}")
            entry = await self.product_entry_repo.get(entry_id)
            if not entry:
    ...
    async def get_entries_by_product(self, product_id: int) -> List[ProductEntry]:
        try:
            logger.info(f"Fetching price entries for product: {product_id}")
            return await self.product_entry_repo.get_by_product(product_id)
        except Exception as e:
            logger.error(f"Error fetching price entries for product {product_id}: {str(e)}")
            raise

    async def get_entries_by_store_brand(self, store_brand_id: int) -> List[ProductEntry]:
        try:
            logger.info(f"Fetching price entries for store brand: {store_brand_id}")
            return await self.product_entry_repo.get_by_store_brand(store_brand_id)
        except Exception as e:
            logger.error(f"Error fetching price entries for store brand {store_brand_id}: {str(e)}")
            raise

    async def get_entries_by_store_location(self, store_location_id: int) -> List[ProductEntry]:
        try:
            logger.info(f"Fetching price entries for store location: {store_location_id}")
            return await self.product_entry_repo.get_by_store_location(store_location_id)
        except Exception as e:
